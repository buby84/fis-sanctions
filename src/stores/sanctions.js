...
export const useSanctionsStore = defineStore('sanctions', {
    ...
    actions: {
         ...
         */
        setFilters(filters) {
            //check if seaspm ois a valid year
            if (filters.season && !/^\d{4}$/.test(filters.season)) {
                this.loadingErrors = 'Invalid year';
                return;
            }
            ...
            if (this.cachedSanctions[key]) {
                ...
                try {
                    ...
                    results.forEach(sanction => {
                        //get athelete gendere from competition summary,
                        //TO-DO: better to implement a call to the athlete api to get the gender?
                        if (sanction.athlete) {
                            sanction.athlete.gender = sanction.competitionSummary.genderCode;
                        }
                        //add discipline code name for results
